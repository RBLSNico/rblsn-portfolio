    ...
    const ProjectCard: React.FC<ProjectCardProps> = ({ item, index, isWorkExperience = false }) => {
        return (
            <div
                ...
                className={`flex flex-col items-center justify-center gap-5 p-10 rounded-xl ${item.gradient} w-full ${isWorkExperience ? 'md:flex-row' : 'md:flex-row-reverse'}`}
                id={item.id}
            >
                {/* Carousel Section */}
                        ...
                        <CarouselContent>
                            {item.images.map((image: string, i: number) => (
                                <CarouselItem key={i} className="flex justify-center">
                                    <img src={image} alt={`${item.title} - ${i + 1}`} className="rounded-xl w-full h-auto" />
                                </CarouselItem>
                            ))}
                        </CarouselContent>
                        <CarouselPrevious />
